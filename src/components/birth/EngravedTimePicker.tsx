  ...
  return (
    <div className={cn('flex flex-col gap-1.5 w-full', className)}>
      <label className="font-heading text-xs font-semibold tracking-wider text-kc-maroon dark:text-kc-gold uppercase flex items-center justify-between">
        ...
        <span className="text-[10px] text-kc-gold-royal lowercase italic">sundial & hora hours</span>
      </label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        ...
        >
          {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (
            <option key={h} value={h} className="bg-kc-paper dark:bg-kc-burnt-brown">
              {h < 10 ? `0${h}` : h} Hour
            </option>
          ))}
        </select>
        ...
        >
          {Array.from({ length: 60 }, (_, i) => i).map((m) => (
            <option key={m} value={m} className="bg-kc-paper dark:bg-kc-burnt-brown">
              {m < 10 ? `0${m}` : m} Min
            </option>
          ))}
        </select>
        ...
        {/* AM / PM Toggle */}
        <div className="flex border border-kc-brass/50 dark:border-kc-gold/30 rounded-xs overflow-hidden">
          <button
            type="button"
            onClick={() => handlePeriodToggle('AM')}
            className={cn(
              'flex-1 py-2 font-heading text-xs font-bold tracking-wider transition-colors cursor-pointer',
              period === 'AM'
                ? 'bg-kc-maroon text-kc-ivory dark:bg-kc-gold dark:text-kc-burnt-brown'
                : 'bg-kc-sand/30 text-kc-text-muted hover:text-kc-maroon'
            )}
          >
            AM (पूर्वाह्न)
          </button>
